 ...
 * tasks once today's budget has room, i.e. after the UTC day rolls over, and queue ready PENDING
 * tasks of approved graphs that nothing queued.
 */
export const tickSweeperProcessor: JobProcessor = async (_job, deps) => {
  const report = await sweep(deps);
  if (report.requeuedStuck + report.failedStuck + report.requeuedBudget + report.queuedReady > 0) {
